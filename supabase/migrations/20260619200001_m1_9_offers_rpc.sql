...
-- =============================================================================
-- SECURITY DEFINER, transactional. Offer is SENSITIVE: write RPCs (submit_offer,
-- issue_offer, respond_to_offer, transition_offer, generate_offer_ref) are
-- service-role-guarded (revoked from public, M3.1) like M1.2; reads
-- (sync_offer_status, offer_detail, list_offers) are authenticated.
--
-- submit_offer is the M0.3 engine's second consumer (submit_workflow('offer',…)).
-- The seeded conditional step (salary_amount >= 2000) is exercised via the
-- context_json salary_amount. respond_to_offer (accept) drives the M1.5 pipeline.
-- audit_log + notify signatures match M0.x. No storage bucket SQL. No recursive CTEs.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. submit_offer — validate, hand off to the workflow engine, store instance.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 2. sync_offer_status — derive offer status from the linked workflow instance.
--    Persist ONLY on a terminal change (+ event, audit, notify). Authenticated.
-- -----------------------------------------------------------------------------
...
begin
  ...
  v_new := case v_inst_status
    ...
    else null            -- in_progress / blocked -> leave (stay in_approval)
  end;
...
-- -----------------------------------------------------------------------------
-- 3. issue_offer — require 'approved'; freeze the bilingual letter snapshot;
--    create an M0.5 document; set esign_status (skipped while dormant). Issue.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 4. respond_to_offer — recruiter records the candidate's accept/decline.
--    accept -> advance the M1.5 application + onboarding_ready; decline -> reason.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 5. transition_offer — cancel | expire (guarded valid transitions).
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 6. offer_detail — offer + application/candidate + instance summary + events.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 7. list_offers — filtered, paged. p_mine = caller is the offer creator.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 8. GRANTS / REVOKES — reads authenticated; sensitive writes service-role (M3.1).
-- -----------------------------------------------------------------------------
...
-- Write RPCs stay service-role-guarded until M3.1 (revoke from public).
